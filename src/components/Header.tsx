...
interface HeaderProps {
  currentUser?: {
    ...
    identityVerification?: { status: string; rejectionReason?: string | null } | null;
  } | null;
}
...
export default function Header({ currentUser }: HeaderProps) {
  ...
  const isCustomerUnapproved = currentUser?.role === 'CUSTOMER' && currentUser?.accountStatus !== 'ACTIVE';

  const triggerLockedModal = (e: React.MouseEvent, title: string, message: string) => {
    if (isCustomerUnapproved) {
      e.preventDefault();
      e.stopPropagation();
      setModalTitle(title);
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            ...
            <nav className="hidden md:flex items-center gap-7 text-xs sm:text-sm font-bold text-[#6D315D]">
              ...
              <Link href={faqsTarget} className="hover:text-[#E94B83] transition-colors">
                FAQs
              </Link>
            </nav>
                    ...
                    {userDropdownOpen && (
                      <div className="absolute right-0 mt-3 w-60 bg-white border border-[#F47B8F]/20 rounded-2xl shadow-xl py-2 z-50 animate-in fade-in slide-in-from-top-2">
                        ...
                        <button
                          ...
                          className="w-full flex items-center gap-2 px-4 py-2.5 text-xs text-rose-600 hover:bg-rose-50 text-left font-bold cursor-pointer"
                        >
      ...
      <AccessLockedModal
        ...
        rejectionReason={currentUser?.identityVerification?.rejectionReason}
      />
    </>
  );
}
